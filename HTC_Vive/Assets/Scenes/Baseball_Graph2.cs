...
public class Baseball_Graph2 : MonoBehaviour {
    //Baseball_Graph 씬의 Graph (1) 오브젝트에 삽입되는 스크립트이다.

    public Transform pointPrefab;
    private int resolution = 0;
    Transform[] points;

    void Start()  //시작 시 getscore함수를 호출한다.
    { 
        getscore();
    }
    ...
    {   //DB로부터 데이터를 받아 온다.
        ...
        {   //받아온 데이터 중 사용자가 여태까지 공을 받아치는데 성공한 확률을 막대그래프 형식으로 표시한다.

            Transform point = Instantiate(pointPrefab);
            point.localScale = scale;

            point.localScale += new Vector3(0, float.Parse(split[j + 2]) / 60, 0);
            point.localScale -= new Vector3(0.015f, 0, 0);
            position.x = (i + 0.5f) * step - 1f;
            
            position.y = float.Parse(split[j + 2]) / 120;

...
public class Soccer_Fire : MonoBehaviour {
    //FirePos오브젝트에 삽입되는 스크립트이다.
    ...
    public static int total_soccercount = 0;
    ...
    void Update(){
        ...
        if ((timer > waitingTime) && (ball_count == 20) && (Score_Text.count >= 15))
        ...
        else if ((timer > waitingTime) && (ball_count == 20) && (Score_Text.count < 15))
    ...
    {  //매 스테이지로 넘어갈 때마다 스코어보드를 초기화한다.
       //스테이지에서 획득한 스코어를 합산 후 다음 스테이지를 표시한다.
        ...
        total_soccercount += Score_Text.count;
        Score_Text.count = 0;
    ...
    {   //게임종료 시 최종 스코어를 합산 후 게임오버씬으로 이동한다.
        ...
        total_soccercount += Score_Text.count;
        SceneManager.LoadScene("SoccerOver");
    }
}

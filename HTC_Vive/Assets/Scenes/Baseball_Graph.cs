...
public class Baseball_Graph : MonoBehaviour {
    //Baseball_Graph 씬의 Graph 오브젝트에 삽입되는 스크립트이다.
    ...
    {   //DB로부터 데이터를 받아 온다.
        ...
        {   //받아온 데이터 중 사용자가 여태껏 받아친 공의 숫자를 막대그래프 형식으로 표시한다.

            Transform point = Instantiate(pointPrefab);
            point.localScale = scale;

            point.localScale += new Vector3(0, float.Parse(split[j + 1]) / 90, 0);
            point.localScale -= new Vector3(0.015f, 0, 0);
            position.x = (i + 0.5f) * step - 1f;

            position.y = float.Parse(split[j + 1]) / 180;
